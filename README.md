...
In this example, we're going to demonstrte how to use *Ably Channels* with the **IF This Then That** API.
...
app.post("/games", (request, response) => {
  const moveOptionsFromClientClickables = createMoveOptions(
    request.body.clickables
  );
...
- We're importing the _npm_ module `node-fetch` at the top
